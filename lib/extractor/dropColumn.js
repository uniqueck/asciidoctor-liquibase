...
function dropColumn (element, currentFile, processModel, logger, vfs) {
  ...
  if (currentFile.endsWith('yaml')) {
    // Rule R01 ---->
    // Trace ---->
    logger.info(`dropColumn - 20230826.183941 - 1 / 3 - ${currentFile} - ${JSON.stringify(processModel)}`)
    // Trace <----
    // Action A01/01: extract ... / yaml / extract table on yaml definition
    const tableName = element.dropColumn.tableName
    const columnName = element.dropColumn.columnName
  ...
  // Condition B01/02: definition is based on ... format / xml / xml definition
    ...
    // Rule R02 ---->
    // Trace ---->
    logger.info(`dropColumn - 20230826.183941 - 2 / 3 - ${currentFile} - ${JSON.stringify(processModel)}`)
    // Trace <----
    ...
    // Rule R03 ---->
    // Trace ---->
    logger.info(`dropColumn - 20230826.183941 - 3 / 3 - ${currentFile} - ${JSON.stringify(processModel)}`)
    // Trace <----
    // Action A02/01: error / X / execute action
    logger.error(`dropColumn - 20230826.183941 - Format for ${currentFile} is not supported.`)
    // Rule R03 <----
  }
