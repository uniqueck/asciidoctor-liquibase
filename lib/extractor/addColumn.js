...
function addColumn (element, currentFile, processModel, logger, vfs) {
  ...
  if (currentFile.endsWith('yaml')) {
    // Rule R01 ---->
    // Trace ---->
    logger.info(`addColumn - 20230826.183854 - 1 / 3 - ${currentFile} - ${JSON.stringify(processModel)}`)
    // Trace <----
  ...
  // Condition B01/02: definition is based on ... format / xml / xml definition
    ...
    // Rule R02 ---->
    // Trace ---->
    logger.info(`addColumn - 20230826.183854 - 2 / 3 - ${currentFile} - ${JSON.stringify(processModel)}`)
    // Trace <----
    ...
    // Rule R03 ---->
    // Trace ---->
    logger.info(`addColumn - 20230826.183854 - 3 / 3 - ${currentFile} - ${JSON.stringify(processModel)}`)
    // Trace <----
    // Action A02/01: error / X / execute action
    logger.error(`addColumn - 20230826.183854 - Format for ${currentFile} is not supported.`)
    // Rule R03 <----
  }
