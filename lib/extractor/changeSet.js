...
function changeSet (element, currentFile, processModel, logger, vfs) {
  ...
  if (currentFile.endsWith('yaml')) {
    ...
    if (element.changeSet.changes) {
      // Rule R01 ---->
      // Trace ---->
      logger.info(`changeSet - 20230826.222749 - 1 / 4 - ${currentFile} - ${JSON.stringify(processModel)}`)
      // Trace <----
      ...
      // Rule R02 ---->
      // Trace ---->
      logger.info(`changeSet - 20230826.222749 - 2 / 4 - ${currentFile} - ${JSON.stringify(processModel)}`)
      // Trace <----
  ...
  // Condition B01/02: definition is based on ... format / xml / xml definition
    ...
    // Rule R03 ---->
    // Trace ---->
    logger.info(`changeSet - 20230826.222749 - 3 / 4 - ${currentFile} - ${JSON.stringify(processModel)}`)
    // Trace <----
    ...
    // Rule R04 ---->
    // Trace ---->
    logger.info(`changeSet - 20230826.222749 - 4 / 4 - ${currentFile} - ${JSON.stringify(processModel)}`)
    // Trace <----
    // Action A02/01: error / X / execute action
    logger.error(`changeSet - 20230826.222749 - Format for ${currentFile} is not supported.`)
    // Rule R04 <----
  }
