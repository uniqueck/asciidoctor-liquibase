...
function addColumn (element, currentFile, processModel, logger, vfs) {
  ...
  if (currentFile.endsWith('yaml')) {
    // Rule R01 ---->
    // Trace ---->
    logger.info(`addColumn - 20230909.210655 - 1 / 3 - ${currentFile} - ${stringify(processModel)}`)
    // Trace <----
    ...
    columns.forEach((column) => {
      const columnDetails = require('./column').extract(column.column, currentFile, logger)
      table.columns.set(columnDetails.name, columnDetails)
    })
    // Rule R01 <----
  // Condition B01/02: definition is based on ... format / xml / xml definition
    ...
    // Rule R02 ---->
    // Trace ---->
    logger.info(`addColumn - 20230909.210655 - 2 / 3 - ${currentFile} - ${stringify(processModel)}`)
    // Trace <----
    ...
    columns.forEach((column) => {
      const columnDetails = require('./column').extract(column, currentFile, logger)
      table.columns.set(columnDetails.name, columnDetails)
    })
    // Rule R02 <----
  } else {
    // Rule R03 ---->
    // Trace ---->
    logger.info(`addColumn - 20230909.210655 - 3 / 3 - ${currentFile} - ${stringify(processModel)}`)
    // Trace <----
    // Action A02/01: error / X / execute action
    logger.error(`addColumn - 20230909.210655 - Format for ${currentFile} is not supported.`)
    // Rule R03 <----
  }
