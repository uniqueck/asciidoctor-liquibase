...
function dropColumn (element, currentFile, processModel, logger, vfs) {
  ...
  if (currentFile.endsWith('yaml')) {
    // Rule R01 ---->
    // Trace ---->
    logger.info(`dropColumn - 20230910.223328 - 1 / 3 - ${currentFile} - ${stringify(processModel)}`)
    // Trace <----
  ...
  // Condition B01/02: definition is based on ... format / xml / xml definition
    ...
    // Rule R02 ---->
    // Trace ---->
    logger.info(`dropColumn - 20230910.223328 - 2 / 3 - ${currentFile} - ${stringify(processModel)}`)
    // Trace <----
    ...
    // Rule R03 ---->
    // Trace ---->
    logger.info(`dropColumn - 20230910.223328 - 3 / 3 - ${currentFile} - ${stringify(processModel)}`)
    // Trace <----
    // Action A02/01: error / X / execute action
    logger.error(`dropColumn - 20230910.223328 - Format for ${currentFile} is not supported.`)
    // Rule R03 <----
  }
