...
function createIndex (element, currentFile, processModel, logger, vfs) {
  ...
  if (currentFile.endsWith('yaml')) {
    // Rule R01 ---->
    // Trace ---->
    logger.info(`createIndex - 20230620.172936 - 1 / 2 - ${currentFile} - ${JSON.stringify(processModel)}`)
    // Trace <----
    ...
    // Rule R02 ---->
    // Trace ---->
    logger.info(`createIndex - 20230620.172936 - 2 / 2 - ${currentFile} - ${JSON.stringify(processModel)}`)
    // Trace <----
    // Action A02: error
    logger.error(`createIndex - 20230620.172936 - Format for ${currentFile} is not supported.`)
    // Rule R02 <----
  }
