...
//
// Generated by LF-ET 2.2.1 (230822A), https://www.lohrfink.de/lfet
// From decision table
...
function renameColumn (element, currentFile, processModel, logger, vfs) {
  ...
  if (currentFile.endsWith('yaml')) {
    // Rule R01 ---->
    // Trace ---->
    logger.info(`renameColumn - 20230823.234714 - 1 / 3 - ${currentFile} - ${JSON.stringify(processModel)}`)
    // Trace <----
  ...
  // Condition B01/02: definition is based on ... format / xml / xml definition
    ...
    // Rule R02 ---->
    // Trace ---->
    logger.info(`renameColumn - 20230823.234714 - 2 / 3 - ${currentFile} - ${JSON.stringify(processModel)}`)
    // Trace <----
    ...
    // Rule R03 ---->
    // Trace ---->
    logger.info(`renameColumn - 20230823.234714 - 3 / 3 - ${currentFile} - ${JSON.stringify(processModel)}`)
    // Trace <----
    // Action A02/01: error / X / execute action
    logger.error(`renameColumn - 20230823.234714 - Format for ${currentFile} is not supported.`)
    // Rule R03 <----
  }
