...
function parseLiquibaseChangeSet(processModel, currentFile, elements, logger, vfs, tillTag) {
    ...
    )
    {
        ...
        )
        {
            ...
            )
            {
                // Rule R01 ---->
                // Trace ---->
                logger.info(`liquibase-parser - 20230611.131058 - 1 / 9 - ${currentFile} - ${JSON.stringify(processModel)}`)
                // Trace <----
            ...
            )
            {
                // Rule R02 ---->
                // Trace ---->
                logger.info(`liquibase-parser - 20230611.131058 - 2 / 9 - ${currentFile} - ${JSON.stringify(processModel)}`)
                // Trace <----
            ...
            else
            {
                // Rule R03 ---->
                // Trace ---->
                logger.info(`liquibase-parser - 20230611.131058 - 3 / 9 - ${currentFile} - ${JSON.stringify(processModel)}`)
                // Trace <----
        ...
        else
        {
            // Rule R04 ---->
            // Trace ---->
            logger.info(`liquibase-parser - 20230611.131058 - 4 / 9 - ${currentFile} - ${JSON.stringify(processModel)}`)
            // Trace <----
            // Action A06/02: next Step / Ex / Exit
            _exit = true
            // Rule R04 <----
    ...
    )
    {
        ...
        if (
        elementDetails.ignore
        )
        {
            // Rule R05 ---->
            // Trace ---->
            logger.info(`liquibase-parser - 20230611.131058 - 5 / 9 - ${currentFile} - ${JSON.stringify(processModel)}`)
            // Trace <----
        ...
        else
        {
            ...
            if (
            processModel.finshedProcessing
            )
            {
                // Rule R06 ---->
                // Trace ---->
                logger.info(`liquibase-parser - 20230611.131058 - 6 / 9 - ${currentFile} - ${JSON.stringify(processModel)}`)
                // Trace <----
                // Action A06/02: next Step / Ex / Exit
                _exit = true
                // Rule R06 <----
            ...
            else
            {
                // Rule R07 ---->
                // Trace ---->
                logger.info(`liquibase-parser - 20230611.131058 - 7 / 9 - ${currentFile} - ${JSON.stringify(processModel)}`)
                // Trace <----
    ...
    else
    {
        ...
        )
        {
            // Rule R08 ---->
            // Trace ---->
            logger.info(`liquibase-parser - 20230611.131058 - 8 / 9 - ${currentFile} - ${JSON.stringify(processModel)}`)
            // Trace <----
            // Action A06/02: next Step / Ex / Exit
            _exit = true
            // Rule R08 <----
        ...
        else
        {
            // Rule R09 ---->
            // Trace ---->
            logger.info(`liquibase-parser - 20230611.131058 - 9 / 9 - ${currentFile} - ${JSON.stringify(processModel)}`)
            // Trace <----
            // Action A06/03: next Step / 1 / Step 1
            _step = 1
            _exit = false
