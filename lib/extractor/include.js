...
function include (element, currentFile, processModel, logger, vfs) {
  ...
  if (currentFile.endsWith('yaml')) {
    ...
    if (element.include.file.endsWith('.yaml')) {
      // Rule R01 ---->
      // Trace ---->
      logger.info(`include - 20230910.223434 - 1 / 7 - ${currentFile} - ${stringify(processModel)}`)
      // Trace <----
    ...
    // Condition B02/02: yaml based definition - file extension / xml / xml
      ...
      // Rule R02 ---->
      // Trace ---->
      logger.info(`include - 20230910.223434 - 2 / 7 - ${currentFile} - ${stringify(processModel)}`)
      // Trace <----
      // Action A02/04: error / MFNS / Mixed liquibase base definitions is not supported.
      logger.error('include - 20230910.223434 - 2 / 7 - Mixed liquibase base definitions is not supported.')
      // Rule R02 <----
    } else {
      // Rule R03 ---->
      // Trace ---->
      logger.info(`include - 20230910.223434 - 3 / 7 - ${currentFile} - ${stringify(processModel)}`)
      // Trace <----
      // Action A02/02: error / UYFE / YAML liquibase definition unknown file extension.
      logger.error('include - 20230910.223434 - 3 / 7 - YAML liquibase definition unknown file extension.')
      // Rule R03 <----
    }
  // Condition B01/02: definition is based on ... format / xml / xml definition
    ...
    if (element.$.file.endsWith('.yaml')) {
      // Rule R04 ---->
      // Trace ---->
      logger.info(`include - 20230910.223434 - 4 / 7 - ${currentFile} - ${stringify(processModel)}`)
      // Trace <----
      // Action A02/04: error / MFNS / Mixed liquibase base definitions is not supported.
      logger.error('include - 20230910.223434 - 4 / 7 - Mixed liquibase base definitions is not supported.')
      // Rule R04 <----
    // Condition B03/02: xml based definition - file extension / xml / xml
    } else if (element.$.file.endsWith('.xml')) {
      // Rule R05 ---->
      // Trace ---->
      logger.info(`include - 20230910.223434 - 5 / 7 - ${currentFile} - ${stringify(processModel)}`)
      // Trace <----
      ...
      // Rule R06 ---->
      // Trace ---->
      logger.info(`include - 20230910.223434 - 6 / 7 - ${currentFile} - ${stringify(processModel)}`)
      // Trace <----
      // Action A02/03: error / UXFE / XML liquibase definition unknown file extension.
      logger.error('include - 20230910.223434 - 6 / 7 - XML liquibase definition unknown file extension.')
      // Rule R06 <----
    }
  } else {
    // Rule R07 ---->
    // Trace ---->
    logger.info(`include - 20230910.223434 - 7 / 7 - ${currentFile} - ${stringify(processModel)}`)
    // Trace <----
    // Action A02/01: error / UDF / Format for '${currentFile}' is not supported.
    logger.error(`include - 20230910.223434 - 7 / 7 - Format for '${currentFile}' is not supported.`)
    // Rule R07 <----
  }
