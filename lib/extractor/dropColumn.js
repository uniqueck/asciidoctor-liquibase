...
function dropColumn (element, currentFile, processModel, logger, vfs) {
  ...
  if (currentFile.endsWith('yaml')) {
    // Rule R01 ---->
    // Trace ---->
    logger.info(`dropColumn - 20230620.150007 - 1 / 3 - ${currentFile} - ${JSON.stringify(processModel)}`)
    // Trace <----
  ...
  // Condition B01/02: definition is based on ... format / xml / xml definition
    ...
    // Rule R02 ---->
    // Trace ---->
    logger.info(`dropColumn - 20230620.150007 - 2 / 3 - ${currentFile} - ${JSON.stringify(processModel)}`)
    // Trace <----
    ...
    // Rule R03 ---->
    // Trace ---->
    logger.info(`dropColumn - 20230620.150007 - 3 / 3 - ${currentFile} - ${JSON.stringify(processModel)}`)
    // Trace <----
    // Action A02: error
    logger.error(`dropColumn - 20230620.150007 - Format for ${currentFile} is not supported.`)
    // Rule R03 <----
  }
