...
function createIndex(element, currentFile, processModel, logger, vfs) {
    ...
    )
    {
        // Rule R01 ---->
        // Trace ---->
        logger.info(`createIndex - 20230611.133323 - 1 / 2 - ${currentFile} - ${JSON.stringify(processModel)}`)
        // Trace <----
    ...
    else
    {
        // Rule R02 ---->
        // Trace ---->
        logger.info(`createIndex - 20230611.133323 - 2 / 2 - ${currentFile} - ${JSON.stringify(processModel)}`)
        // Trace <----
        // Action A02: error
        logger.error(`createIndex - 20230611.133323 - Format for ${currentFile} is not supported.`)
        // Rule R02 <----
    }
